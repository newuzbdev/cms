...
(function () {
  ...
  function renderBlockCard(block, index) {
    ...
    if (block.type === 'header') {
      ...
      f += renderInput(block.id, 'logoTagline', 'Подзаголовок логотипа', block.logoTagline);
      ...
      f += renderImage(block.id, 'image', 'Основное изображение', block.image);
      f += renderImage(block.id, 'imageSmall', 'Маленькое изображение (слева от основного)', block.imageSmall);
...
.admin__btn_red:hover {
    background: #fee2e2;
}
`;

if(!adminCss.includes('admin__array-editor')) {
    fs.appendFileSync('admin.css', appendingCss);
}
